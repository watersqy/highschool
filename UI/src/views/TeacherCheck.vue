...
import {teacherDepartmentListService, teacherListService} from "@/api/user.js";
import {teacherTableService} from "@/api/course.js";
...
const courseList=async (row)=>{
  ...
  Visible.value=true
  title.value=row.id;
  let result = await teacherTableService(row.id);
